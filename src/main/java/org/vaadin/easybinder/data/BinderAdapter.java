...
public class BinderAdapter<BEAN> extends Binder<BEAN>{
    ...
    public void writeBean(BEAN bean) throws ValidationException {
		if(!binder.isValid()) {
			throw new ValidationException(
					binder.bindings.stream().map(e -> e.validate()).filter(e -> e.isError()).collect(Collectors.toList()),
					binder.constraintViolations.stream().filter(e -> e.getPropertyPath().toString().equals("")).map(e -> ValidationResult.error(e.getMessage())).collect(Collectors.toList())
					);
		}
    ...
    public BinderValidationStatus<BEAN> validate() {
		return new BinderValidationStatus<BEAN>(
				this, 
